...
class ServicesController extends Controller
{
    ...
    public function list(ListServicesRequest $request)
    {
        ...
        $cacheKey = 'services_' . serialize($params);
        $minutes = 60; // Тривалість кешування в хвилинах
    
        $items = Cache::remember($cacheKey, $minutes, function () use ($params) {
            $query = ServicesQuery::make($params)->query();
    
            return $query->paginate($params['per_page'] ?? config('filters.per_page'));
        });
